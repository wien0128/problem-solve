...
private:
    ...
    queue<int> q;                   // 인접 정점 큐
    vector<int> visited_order;      // 정점 방문 순서
    int current_order;              // 현재 방문 순서
public:
    // 생성자, 1-based-indexing
    Graph(int n) : adj_list(n + 1), visited_order(n + 1, 0), current_order(1){}
    ...
    void bfs(int node) {
        ...
        visited_order[node] = current_order++;   // 시작 노드는 항상 첫 번째 방문
        q.push(node);
        ...
        while (!q.empty())
        {
            ...
            {
                // 이전에 방문했는가?
                if (!visited_order[neighbors])
                {
                    q.push(neighbors); // 인접 정점 push
                    visited_order[neighbors] = current_order++; // 방문 순서 기록
                }
            }
        }
    }
    ...
    {
        for (size_t i = 1; i < visited_order.size(); ++i)
        {
